...
function handleImgError(e: React.SyntheticEvent<HTMLImageElement>) {
  ...
  if (parent && !parent.querySelector('.img-fallback')) {
    ...
    fallback.style.cssText = 'position:absolute;inset:0;background:linear-gradient(135deg,#1a1a1a,#0a0a0a)';
...
const labelClass = 'block font-sans text-xs uppercase tracking-widest text-luxury-beige/60 mb-2';
  ...
  const handleWhatsAppOrder = () => {
    ...
    const itemLines = items.map(item => {
      ...
      if (item.customisation) {
        const parts: string[] = [];
        if (item.customisation.ribbonColor) parts.push(`Ribbon: ${item.customisation.ribbonColor}`);
        if (item.customisation.packagingStyle) parts.push(`Packaging: ${item.customisation.packagingStyle}`);
        if (item.customisation.greetingCard) parts.push('Greeting Card (+₹50)');
        if (item.customisation.giftMessage) parts.push(`Message: "${item.customisation.giftMessage}"`);
    ...
    window.open(`https://wa.me/917045899262?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer');
  };
    ...
    <div className="min-h-screen bg-luxury-black">
      ...
      <main className="pt-28 pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Header */}
          <div className="mb-12">
            <p className="font-sans text-luxury-gold text-xs tracking-[0.5em] uppercase mb-3">Shopping</p>
            <h1 className="font-serif text-3xl md:text-4xl text-luxury-beige mb-4">Your Cart</h1>
          ...
          {items.length === 0 && (
            <div className="text-center py-24 border border-luxury-gold/10">
              ...
              <h2 className="font-serif text-2xl text-luxury-beige mb-4">Your cart is empty</h2>
              <p className="font-sans font-light text-luxury-beige/50 text-sm mb-10 max-w-xs mx-auto">
                Discover our handcrafted luxury date chocolates and find the perfect gift.
              </p>
              <a
                href="/#products"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-10">
              ...
              <div>
                <div className="space-y-4 mb-8">
                  {items.map(item => {
                    ...
                    if (item.customisation?.ribbonColor) custParts.push(`Ribbon: ${item.customisation.ribbonColor}`);
                    if (item.customisation?.packagingStyle) custParts.push(item.customisation.packagingStyle);
                    if (item.customisation?.greetingCard) custParts.push('Greeting Card (+₹50)');

                    return (
                      <div key={item.id} className="flex gap-4 p-5 border border-luxury-gold/10 hover:border-luxury-gold/25 transition-all duration-300 bg-[#0d0d0d]">
                        {/* Image */}
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-serif text-luxury-beige text-lg mb-0.5">{item.name}</h3>
                          {custParts.length > 0 && (
                            <p className="font-sans text-luxury-gold/70 text-xs italic mb-1">{custParts.join(' • ')}</p>
                          )}
                          ...
                          <div className="flex items-center gap-4 flex-wrap mt-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-sans text-luxury-beige text-sm w-6 text-center">{item.quantity}</span>
                              <button
                                type="button"
                                onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                className="w-7 h-7 border border-luxury-gold/30 text-luxury-beige/70 hover:border-luxury-gold hover:text-luxury-gold transition-colors flex items-center justify-center"
                            ...
                            <span className="font-sans text-luxury-beige/60 text-sm">
                              ...
                              <span className="text-luxury-gold font-semibold ml-1">₹{subtotal.toLocaleString('en-IN')}</span>
                            </span>
                          </div>
                        </div>
                ...
                <div className="mt-10 p-8 border border-luxury-gold/10 bg-[#0d0d0d]">
                  <h2 className="font-serif text-xl text-luxury-beige mb-2">Delivery Details</h2>
                  <div className="w-10 h-px bg-luxury-gold/40 mb-6" />

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="cart-name" className={labelClass}>Full Name *</label>
                      <input
                        id="cart-name"
                        name="name"
                        type="text"
                      ...
                      />
                      {errors.name && <p className="mt-1 text-red-400/80 text-xs">{errors.name}</p>}
                    </div>

                    <div>
                      <label htmlFor="cart-phone" className={labelClass}>Phone Number *</label>
                      <input
                        id="cart-phone"
                        name="phone"
                        type="tel"
                      ...
                      />
                      {errors.phone && <p className="mt-1 text-red-400/80 text-xs">{errors.phone}</p>}
                    </div>

                    <div className="sm:col-span-2">
                      <label htmlFor="cart-email" className={labelClass}>Email Address</label>
                      <input
                        id="cart-email"
                        name="email"
                        type="email"
                    ...
                    <div className="sm:col-span-2">
                      <label htmlFor="cart-address" className={labelClass}>Street Address *</label>
                      <input
                        id="cart-address"
                        name="address"
                        type="text"
                      ...
                      />
                      {errors.address && <p className="mt-1 text-red-400/80 text-xs">{errors.address}</p>}
                    </div>

                    <div>
                      <label htmlFor="cart-city" className={labelClass}>City *</label>
                      <input
                        id="cart-city"
                        name="city"
                        type="text"
                      ...
                      />
                      {errors.city && <p className="mt-1 text-red-400/80 text-xs">{errors.city}</p>}
                    </div>

                    <div>
                      <label htmlFor="cart-state" className={labelClass}>State</label>
                      <input
                        id="cart-state"
                        name="state"
                        type="text"
                    ...
                    <div>
                      <label htmlFor="cart-pincode" className={labelClass}>Pincode *</label>
                      <input
                        id="cart-pincode"
                        name="pincode"
                        type="text"
                      ...
                      />
                      {errors.pincode && <p className="mt-1 text-red-400/80 text-xs">{errors.pincode}</p>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="border border-luxury-gold/15 bg-[#0d0d0d] p-8 sticky top-28">
                  <h2 className="font-serif text-xl text-luxury-beige mb-6">Order Summary</h2>

                  <div className="space-y-3 mb-6 pb-6 border-b border-luxury-gold/10">
                    {items.map(item => {
                      const greetingExtra = item.customisation?.greetingCard ? 50 : 0;
                      const subtotal = (item.price + greetingExtra) * item.quantity;
                      return (
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex justify-between">
                      <span className="font-sans text-sm text-luxury-beige/60">Subtotal</span>
                      <span className="font-sans text-sm text-luxury-beige">₹{totalPrice.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-sans text-sm text-luxury-beige/60 flex items-center gap-2">
                        <Truck className="w-3.5 h-3.5" /> Shipping
                      </span>
                      <span className="font-sans text-xs text-luxury-gold tracking-widest uppercase">Free</span>
                    </div>
                  </div>

                  <div className="border-t border-luxury-gold/20 pt-4 mb-6">
                    <div className="flex justify-between items-center">
                      <span className="font-sans text-sm uppercase tracking-widest text-luxury-beige/70">Total</span>
                      <span className="font-serif text-2xl text-luxury-gold">₹{totalPrice.toLocaleString('en-IN')}</span>
                    </div>
                  </div>
