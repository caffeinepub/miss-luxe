...
import { useCart, CartItemCustomisation } from '../context/CartContext';
...
const PACKAGING_STYLES = ['Standard', 'Premium Velvet', 'Seasonal Edition'] as const;
...
export default function ProductDetailPage({ onCartOpen }: ProductDetailPageProps) {
    ...
    return (
      <div className="min-h-screen bg-luxury-black flex items-center justify-center">
        ...
        <div className="text-center">
          <p className="font-serif text-2xl mb-4 text-luxury-beige">Product not found</p>
          <button
            type="button"
            onClick={() => navigate({ to: '/' })}
  ...
  const handleWhatsApp = () => {
    ...
    lines.push('');
    lines.push('*Product:*');
    lines.push(`- ${product.name} x${qty} — ₹${totalCost.toLocaleString('en-IN')}`);
    lines.push('');
    if (giftMessage.trim() || ribbonColor !== 'Gold' || packagingStyle !== 'Standard' || greetingCard) {
      ...
      if (giftMessage.trim()) lines.push(`  Gift Message: "${giftMessage.trim()}"`);
      lines.push('');
    }
    ...
    window.open(`https://wa.me/917045899262?text=${encodeURIComponent(lines.join('\n'))}`, '_blank', 'noopener,noreferrer');
  };
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
              ...
              <div className="relative group">
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-zinc-900 via-black to-amber-950/40" style={{ aspectRatio: '1/1' }}>
                  <img
                    src={product.image}
                    alt={product.name}
              ...
              <div className="flex flex-col justify-center">
                ...
                <p className="font-serif text-luxury-beige/80 text-lg italic mb-5">{product.tagline}</p>
                ...
                <p className="font-sans text-xs text-luxury-beige/40 mb-5">
                  {product.pieces} pieces · Prices inclusive of premium packaging
                </p>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {product.flavours.map(f => (
                        <span key={f} className="font-sans text-[10px] tracking-widest uppercase text-luxury-gold border border-luxury-gold/30 px-3 py-1 bg-luxury-gold/5">
                          {f}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mb-6">
                  ...
                  <p className="font-sans font-light text-sm leading-8 text-luxury-beige/90">
                    {activeTab === 'description' ? product.description : product.story}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="flex items-center gap-3">
                    <span className="font-sans text-xs uppercase tracking-widest text-luxury-beige/50">Qty</span>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-sans text-luxury-beige text-base font-medium w-8 text-center">{qty}</span>
                      <button
                        type="button"
                  ...
                  {qty > 1 && (
                    <p className="font-sans text-luxury-beige/60 text-sm">
                      Total: <span className="text-luxury-gold font-semibold text-base">₹{totalCost.toLocaleString('en-IN')}</span>
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-6 mb-4 bg-luxury-black border border-luxury-gold/10">
                  <div className="flex flex-col items-center text-center gap-2">
                    ...
                    <span className="text-xs tracking-[0.1em] uppercase font-medium text-luxury-beige/50">Net Weight</span>
                    <span className="font-sans text-sm font-medium text-luxury-beige">{product.netWeight}</span>
                  </div>
                  <div className="flex flex-col items-center text-center gap-2">
                    <Clock size={18} className="text-luxury-gold" />
                    <span className="text-xs tracking-[0.1em] uppercase font-medium text-luxury-beige/50">Shelf Life</span>
                    <span className="font-sans text-sm font-medium text-luxury-beige text-center">{product.shelfLife}</span>
                  </div>
                  <div className="flex flex-col items-center text-center gap-2">
                    <Thermometer size={18} className="text-luxury-gold" />
                    <span className="text-xs tracking-[0.1em] uppercase font-medium text-luxury-beige/50">Storage</span>
                    <span className="font-sans text-xs text-center text-luxury-beige/70">{product.storageInstructions}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-16 p-8 lg:p-12 border border-luxury-gold/15 bg-[#0d0d0d]">
              <div className="flex items-center gap-4 mb-2">
                ...
                <h2 className="font-serif text-2xl font-bold text-luxury-beige">Personalise Your Box</h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  />
                  <p className="font-sans text-luxury-beige/30 text-xs mt-1 text-right">{giftMessage.length}/120</p>
                </div>

                <div>
                  {/* Ribbon Color */}
                  <p className="font-sans text-xs uppercase tracking-[0.15em] text-luxury-gold mb-3">Ribbon Colour</p>
                  ...
                  {/* Packaging Style */}
                  <p className="font-sans text-xs uppercase tracking-[0.15em] text-luxury-gold mb-3">Packaging Style</p>
                ...
                >
                  <span
                    className={`w-4 h-4 border-2 flex items-center justify-center transition-colors ${
                      greetingCard ? 'border-luxury-black bg-luxury-black' : 'border-luxury-gold/50'
                    }`}
                  >
                    {greetingCard && <span className="text-luxury-gold text-[10px] font-bold leading-none">✓</span>}
                  </span>
            ...
            <div className="mt-8 p-8 lg:p-12 bg-[#0e0e0e] border border-luxury-gold/10">
              <h2 className="font-serif text-2xl font-bold text-luxury-beige mb-2">Ingredients</h2>
              <div className="w-12 h-px bg-luxury-gold/50 mb-6" />
              <p className="font-sans font-light text-sm text-luxury-beige/80 mb-6">
                Every ingredient is carefully sourced to meet our exacting standards of quality and purity.
              </p>
            ...
            <div className="mt-8 p-8 border border-luxury-gold/10 bg-luxury-black">
              <h2 className="font-serif text-xl font-bold text-luxury-beige mb-4">What's Included</h2>
              <p className="font-sans font-light text-sm text-luxury-beige/70 leading-relaxed">{product.includes}</p>
            </div>
