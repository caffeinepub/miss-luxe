...
export default function AboutSection() {
  return (
    <section id="about" className="py-20 md:py-32 bg-luxury-black border-t border-luxury-gold/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  if (parent && !parent.querySelector('.img-fallback')) {
                    ...
                    fallback.style.cssText = 'background:linear-gradient(135deg,#1a1000 0%,#0a0a0a 50%,#1a1a00 100%)';
          ...
          <Reveal className="order-1 md:order-2 space-y-6">
            <div>
              <p className="font-sans text-luxury-gold text-xs tracking-[0.5em] uppercase mb-4">A Story of Craft & Devotion</p>
            ...
            <p className="font-sans font-light text-luxury-beige/90 leading-relaxed">
              Miss Luxe was born from a simple belief — that the finest gifts are made with the finest hands. We source only the plumpest Medjool dates, pair them with globally sourced fillings, and wrap them in packaging that speaks before words can.
            </p>
            <p className="font-sans font-light text-luxury-beige/90 leading-relaxed">
              Every box is a labour of love, produced in limited batches to ensure every piece meets our uncompromising standard of excellence. We do not compromise. We do not rush. We make what we would be proud to give to someone we love.
            </p>
            ...
            <div className="grid grid-cols-3 gap-6 pt-4 border-t border-luxury-gold/10">
              {[
                ...
                <div key={stat.label} className="text-center">
                  <div className="font-display text-2xl text-luxury-gold tracking-wider">{stat.value}</div>
                  <div className="font-sans text-luxury-beige/50 text-xs tracking-widest uppercase mt-1">{stat.label}</div>
                </div>
              ))}
            </div>
        ...
        <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-0 border border-luxury-gold/10">
          {[
            { icon: '✦', label: 'Handcrafted with Love', desc: 'Every date filled and finished by hand' },
            { icon: '◈', label: 'Premium Medjool Dates', desc: 'Sourced for richness and plumpness' },
            { icon: '⟡', label: 'Ships Pan India', desc: 'Delivered in 3–5 working days' },
            { icon: '◇', label: 'Prepaid Orders Only', desc: 'No COD — quality assured production' },
          ].map((item, i) => (
            <div
              key={item.label}
            ...
            >
              ...
              <p className="font-sans text-luxury-beige text-xs tracking-widest uppercase mb-1 font-medium">{item.label}</p>
              <p className="font-sans text-luxury-beige/45 text-xs">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
