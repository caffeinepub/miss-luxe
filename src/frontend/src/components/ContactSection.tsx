...
export default function ContactSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.open(`https://wa.me/917045899262?text=${encodeURIComponent(text)}`, '_blank');
    setSubmitted(true);
  };
    ...
    return (
      <section id="contact" className="py-20 md:py-32 bg-luxury-black">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <h2 className="font-serif text-3xl text-luxury-beige mb-4">Thank You</h2>
          <p className="font-sans text-luxury-beige/60 font-light">
            Your inquiry has been sent. We'll get back to you shortly via WhatsApp.
          </p>
          <button
            type="button"
    ...
    <section id="contact" className="py-20 md:py-32 bg-luxury-black">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <p className="font-sans text-luxury-gold text-xs tracking-[0.5em] uppercase mb-4">Get in Touch</p>
          <h2 className="font-serif text-3xl md:text-4xl text-luxury-beige mb-6">Send an Inquiry</h2>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          <div>
            ...
            />
            {errors.name && <p className="mt-1 text-red-400/80 text-xs font-sans">{errors.name}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.email && <p className="mt-1 text-red-400/80 text-xs font-sans">{errors.email}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.message && (
              <p className="mt-1 text-red-400/80 text-xs font-sans">{errors.message}</p>
            )}
          </div>

          <button type="submit" className="btn-gold w-full py-4 text-sm tracking-widest">
            Send Inquiry
          </button>
        </form>
      </div>
